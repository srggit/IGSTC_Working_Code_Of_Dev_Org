angular.module('cp_app').controller('WORKSHOPDocUpload_Ctrl', function($scope,$rootScope) {
    $scope.objContact={};
    $scope.Traveller={};
    $scope.Visa={};
    ...
    $scope.saveBankDetails = function(){
        ...
        "Bank_SWIFT_Code__c":$scope.bankDetails.Bank_SWIFT_Code__c,"Bank_Branch_Name__c":$scope.bankDetails.Bank_Branch_Name__c,"Bank_Address__c":$scope.bankDetails.Bank_Address__c}
        ApplicantPortal_Contoller.saveBankDetails($scope.bankDetails2,function(result,event){
            debugger;
            if(event.status && result){
                debugger;
                swal('success','Invoice & Bank details have been saved successfully.','success');
            }
            $scope.$apply();
        })
    }
        ...
        ApplicantPortal_Contoller.getContactUserDoc($rootScope.contactId, function (result, event) {
            ...
            if (event.status) {
                ...
                for(var i=0;i<$scope.allDocs.length;i++){
                    if($scope.allDocs[i].userDocument.Name == 'Invoice form (Only for German location)'){
    ...
    $scope.updateProposal=function(type){
        if(type == 'Invoice Form'){
            if($scope.objContact.Account_Country_Type__c=="India"){
